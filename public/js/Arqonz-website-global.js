...
document.addEventListener('DOMContentLoaded', () => {
    ...
    mobileMenus.forEach(menu => {
        menu.addEventListener('click', function(event) {
            ...
            // Handle Sub-Children-Menu (sub-submenu) for mobile
            const subChildrenMenus = parentMenu.querySelectorAll('.Sub-Children-Menu');
            subChildrenMenus.forEach(subMenu => {
                const subParent = subMenu.closest('li');
                subParent.addEventListener('click', (event) => {
                    event.preventDefault();
                    const isSubActive = subParent.classList.contains('active');

                    document.querySelectorAll('.Sub-Children-Menu').forEach(sub => {
